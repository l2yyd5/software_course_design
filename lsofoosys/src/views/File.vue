<template>
  <el-container>
    <el-header style="font-size:32px;color:white">File Hook</el-header>
    <el-main>
...
export default {
  ...
  methods: {
    getData(){
      setInterval(()=>{
        if(this.Gstatus === 'active'){
          axios.get("http://127.0.0.1:23333/FiAction").then((res) =>{
    ...
    clearData(){
      ...
      this.expandedList4 = []
      axios.get("http://127.0.0.1:23333/FiAction?action=cleardata").then((res) =>{
          console.log(res)
        }).catch(err => {
          console.log(err)
        });
    ...
    nodeExpand(data) {
        this.expandedList4.push(data.id)
    },
    nodeCollapse(data) {
      this.expandedList4.splice(this.expandedList4.indexOf(data.id), 1);
    },
  },
